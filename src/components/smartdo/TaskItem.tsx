...
export function TaskItem({ task }: TaskItemProps) {
  ...
  return (
    <Card className={cn("transition-all duration-300 ease-in-out", task.completed ? "bg-card/60 opacity-70" : "bg-card shadow-md hover:shadow-lg", isOverdue && !task.completed ? "border-destructive/50 ring-2 ring-destructive/30" : "")}>
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DialogTrigger asChild onOpenChange={setIsEditDialogOpen}>
                 <DropdownMenuItem onSelect={(e) => e.preventDefault()}> {/* Prevent auto close for DialogTrigger */}
              ...
              <SmartReminderDialog task={task}>
                <DropdownMenuItem onSelect={(e) => e.preventDefault()}>
                  {task.reminderDateTime ? <Bell className="mr-2 h-4 w-4 text-accent" /> : <BellPlus className="mr-2 h-4 w-4" />}
          ...
          <CategoryPill category={task.category} />
           {task.completed ? (
            ...
            <div className={cn("flex items-center", isOverdue ? "text-destructive font-semibold" : "")}>
              {isOverdue ? <AlertTriangle className="h-3.5 w-3.5 mr-1" /> : <CalendarDays className="h-3.5 w-3.5 mr-1" />}
              <span>
                {isOverdue
      ...
      {/* Dialog for Editing Task - controlled by isEditDialogOpen state */}
...
// Helper for AlertDialog destructive action button style
